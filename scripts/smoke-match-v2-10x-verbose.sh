...
for i in $(seq 1 "$N"); do
  ...
  LEAD="$($CURL -X POST "$BASE/leads" -H "Content-Type: application/json" -d "{\"initialText\":\"smoke v2 $i\"}")"
  LEAD_ID="$(echo "$LEAD" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])")"
  echo "    leadId=$LEAD_ID"

  echo "    wizard: answering"
  # Wizard: max 10 adım güvenlik
  for step in $(seq 1 10); do
    ...
    echo "      step $step: key=$KEY ans=$ans"
    $CURL -X POST "$BASE/leads/$LEAD_ID/wizard/answer" \
      -H "Content-Type: application/json" \
      -d "{\"key\":\"$KEY\",\"answer\":\"$ans\"}" >/dev/null
  done
