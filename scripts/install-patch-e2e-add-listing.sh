...
# patch script'ini üret
cat > "$ROOT/scripts/patch-dev-start-and-e2e-add-listing.sh" <<'PATCH'
#!/usr/bin/env bash
# ÇALIŞILACAK KÖK: ~/Desktop/teklif-platform
set -euo pipefail
...
# Ekleme noktası: match() sonrasında (ASSIGNED döndükten sonra) "Özet:"ten önce.
# En güvenlisi: "-> match()" satırını bulup, hemen altına eklemek.
m = re.search(r"(?m)^\s*->\s*match\(\)\s*$", txt)
if not m:
    raise SystemExit("❌ dev-start-and-e2e.sh içinde '-> match()' bulunamadı.")

# insert: match satırının bitiminden sonraki ilk yeni satırdan sonra
insert_pos = txt.find("\n", m.end()) + 1
...
print("✅ dev-start-and-e2e.sh patchlendi: listing create+link+verify eklendi.")
PY
...
chmod +x "$ROOT/scripts/patch-dev-start-and-e2e-add-listing.sh"

echo "✅ Installer OK."
echo "Şimdi patch uygula:"
echo "  cd $ROOT && bash ./scripts/patch-dev-start-and-e2e-add-listing.sh"
