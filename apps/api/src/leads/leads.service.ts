...
export class LeadsService {
  ...
  async upsertAnswer(id: string, key: string, answer: string) {
    ...
    // ANSDEAL_UPSERT_V1
    try {
      console.log('ANSDEAL_IN', { leadId: id, key: key, answer: answer });
    } catch (e) {}
    if (key && answer) {
      const data: any = {};
      ...
      if (Object.keys(data).length) {
        ...
        console.log('ANSDEAL_UPSERT_OK', after);
      } else {
        console.log('ANSDEAL_SKIP_KEY', k);
      }
    } else {
      console.log('ANSDEAL_SKIP_EMPTY');
    }
  ...
  async wizardAnswer(leadId: string, key?: string, answer?: string) {
    // WIZ_TO_UPSERT_UNIFY_V1
    // Single source of truth: reuse upsertAnswer persist logic
    return this.upsertAnswer(leadId, key as any, answer as any);
  }
