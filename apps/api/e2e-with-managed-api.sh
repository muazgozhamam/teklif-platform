...
AUTO_STOP="${AUTO_STOP:-0}"   # AUTO_STOP=1 verirsen test bitince API'yi kapatır
...
if [ "$ok" != "1" ]; then
  echo "HATA: API ayağa kalkmadı. Log:"
  tail -n 160 "$LOG" || true
...
echo
echo "==> 5) E2E: Lead create -> Deal by lead -> Advance"
