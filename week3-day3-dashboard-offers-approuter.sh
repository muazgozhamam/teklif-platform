...
if [ ! -d "$APP_DIR" ]; then
  echo "ERROR: App Router klasörü yok: $APP_DIR"
  echo "Dashboard dizinleri:"
  (cd "$DASH" && find . -maxdepth 2 -type d -print)
  exit 1
fi

# Sende zaten apps/dashboard/lib var görünüyor; yoksa oluştur.
mkdir -p "$DASH/lib"
mkdir -p "$APP_DIR/offers/provider"
mkdir -p "$APP_DIR/offers/customer"

# API helper (dashboard/lib/api.ts)
cat > "$DASH/lib/api.ts" <<'TS'
